...
fetch("Liste_Pays.json") // chemin du fichier
    ...
    .then(data => {
        function chouse_countrie(nbr_pays,score,mauvais_pays,nbr_essais,change_colore ,points_perdue){
        const tab_pays = data;
        ...
        var premierPays = tab_pays['Amérique'][aleatoire]; //const premierPays = tab_pays["Amérique"][aleatoire]; 
        ...
        paths.forEach(function(path) {
            path.addEventListener('click', function(event) {
                ...
                if(id_map === random_id){//Comparer l'id du map et l'id cliké sur le map et l'id du mot rechercher aleatoirement
                    ...
                    score ++;
                    premierPays = tab_pays['Amérique'][aleatoire]; // premierPays = tab_pays["Amérique"][aleatoire]; cette variable reçois la l'élément du tableau sur lequel nous allons travailler! 
                    ...
                    //premierPays = tab_pays['Amérique'][aleatoire];
                    var index_existe = tab_pays['Amérique'].findIndex(country => country.ref === random_id);
                    if(index_existe !== -1){//Verifi si l'index de l'élément trouver à travers le bon click existe dans le tableau
                        ...
                        tab_pays['Amérique'].splice(index_existe,1);//Si l'élément rechercher existe il sera supprimer à travers la fonction splite;
                        ...
                        console.log("Random id:"+random_id);
                        nbr_essais = 0;
                        tab_pays['Amérique'].forEach(county =>{
                    ...
                    console.log("Mauvais pays.");
                     ...
                     if(nbr_essais>= 3){
                        ...
                        if(index_existe !== -1){
                            console.log('Pays supprimé :',random_id);
                            tab_pays['Amérique'].splice(index_existe,1);//Supprimer element du tableau;
                            points_perdue++;
                            
                            aleatoire = Actualiser_Variable_Aleatoire(nbr_pays);//Apres avoir trouver le pays actualiser le score
                            premierPays = tab_pays['Amérique'][aleatoire]; //const premierPays = tab_pays["Amérique"][aleatoire]; 
    ...
    var nbr_pays = 35, score=0; var mauvais_pays = false; var nbr_essais = 0; var change_colore="black";var points_perdue= 0;
    mauvais_pays = chouse_countrie(nbr_pays,score,mauvais_pays,nbr_essais,change_colore, points_perdue);
    if(mauvais_pays){
        for( i = 0;i<2;i++){
